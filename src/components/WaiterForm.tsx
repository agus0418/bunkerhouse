...
export default function WaiterForm({ waiter, onClose }: WaiterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const waiterData = {
      ...
      totalTips: formData.totalTips,
      createdAt: waiter?.createdAt || new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };

    try {
      if (waiter) {
        await setDoc(doc(waitersRef, waiter.id), waiterData);
      } else {
        const newWaiterRef = doc(waitersRef);
        await setDoc(newWaiterRef, { ...waiterData, id: newWaiterRef.id });
      }
      onClose();
    } catch (error) {
